...
public class PlugInSnr58Euclidean extends Pluggable implements PipelinePlugIn{
	...
	private ArrayList<PipelineNoteObject> addRhythmPositions(PipelineNoteList pnl){
		...
		for (double pos = 0.0; pos < pnl.length; pos += repLength){
			for (double pos2 = 0.0; pos2 < repLength; pos2 += inc){
				...
				if (actualpos < pnl.length && snrPosNotTaken(pnl, actualpos)){
					pnoList.add(new PipelineNoteObject(actualpos, true, true, DrumStaticVariables.snrDescriptor));
				}	
			}					
		}
		return pnoList;
