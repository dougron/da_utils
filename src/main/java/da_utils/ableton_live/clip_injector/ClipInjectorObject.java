...
/*
 * 
 * this is a duplicate of the SortaSelekta ClipInjectorObject with the semdNode(Node node) method deleted
 * should be added to DAUtils, but am lazy. Actually is in DAUtils now. all static stuff has been removed
 * so we can use multiple cios on multiple ports
 */
public class ClipInjectorObject {
	...
	public static StaticUDPConnection conn = new StaticUDPConnection(port);
	...
	public void sendClip(LiveClip lc){
		...
		sendPitchBendMessage(lc);
		sendControllerMessages(lc.cList);
	}
	public void sendPitchBendMessage(LiveClip lc) {
		if (lc.pb != null) {
			OSCMessMaker mess = new OSCMessMaker();
			mess.addItem(injectMessage);
			addPitchBendClipToOSCMessage(lc.pb, mess);
			conn.sendUDPMessage(mess);
		}		
	}
	public void sendInitializationMessage(){
		OSCMessMaker mm = new OSCMessMaker();
		mm.addItem(initMessage);
	...
	public void sendResetInitializationMessage(){
		// will reset the entire batch of ClipObjects in Live, hence static, erm actually not for some reason
		OSCMessMaker mess = new OSCMessMaker();	
